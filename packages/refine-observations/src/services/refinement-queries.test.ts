...
describe('RefinementQueries', () => {
  ...
  it('should create daily refined observations table query', () => {
    const subject = RefinementQueries.createDailyRefinedTable();

    expect(subject).toContain('CREATE EXTERNAL TABLE IF NOT EXISTS observations_refined_daily');
    expect(subject).toContain("LOCATION 's3://weather-tempest-records/refined/observations_refined_daily/'");
  });
  ...
  it('should create daily insert query for a specific date', () => {
    const subject = RefinementQueries.insertDailyRefinedRowsForDate({ year: '2026', month: '02', day: '14' });

    expect(subject).toContain('INSERT INTO observations_refined_daily');
    expect(subject).toContain("AND day='14'");
  });
});
